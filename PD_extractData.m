% Convert ABET data from pairwise discrimination experiments to separate files
% per schedule groupID & animalID
% Takes csv data from
clc;clear all;

filepath = 'C:\Users\u0105250\Documents\mouse Touchscreen Exp\';
filename = 'mousePD_group135.csv';
outputPath = 'C:\Users\u0105250\Documents\mouse Touchscreen Exp\';

useExcel = 0;
saveFile = 2; % 1 = create subdir for group/animalID/; 2 = use group_animalID in the filename
...
for rowIndex = 1:size(alldata,1)
   ...
   %distractorIndex = cell2mat(alldata(rowIndex,   (responseOffset + 4*maxResponses):(responseOffset + 4*maxResponses + nResponses - 1)));
   correctionTrial = cell2mat(alldata(rowIndex,   (responseOffset + 5*maxResponses):(responseOffset + 5*maxResponses + nResponses - 1)));
   
   % Extract RT data
   rtOffset = responseOffset + 6*maxResponses;
   ...
   if saveFile == 1
       ...
       fid = fopen([outputPath schedule '\' groupID '\' animalID '\' year month day '.csv'],'w');
       for i = 1:nResponses
           fprintf(fid,[num2str(i) ',' num2str(responseEvaluations(i)) ','  ...
